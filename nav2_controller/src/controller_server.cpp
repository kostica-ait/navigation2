...
ControllerServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  tracking_feedback_pub_ = create_publisher<nav2_msgs::msg::TrackingFeedback>(
...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  if (current_path_.poses.size() >= 2) {
    ...
    tracking_feedback_msg->speed = std::hypot(
      cmd_vel_2d.twist.linear.x, cmd_vel_2d.twist.linear.y);
